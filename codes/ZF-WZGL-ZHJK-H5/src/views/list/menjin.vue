      ...
      <van-pull-refresh v-if="list.length > 0" v-model="refreshing" @refresh="onRefresh" style="background: #E5E5E5;padding: 20px 20px 0;">
        <van-list v-model="loading" :finished="finished" :finished-text="finishedtext" loading-text="正在加载" @load="onLoad" offset="30" :immediate-check="false">
            <van-cell v-for="(item,index) in list" :key="index" @click="godetails('details',item)">
                <div class="listtop">
                    <div class="listtitle">{{item.visitReason}}</div>
...
import {approval,subscribeqx} from '@/api/api'
...
export default {
  ...
  created () {
    ...
    this.userId = store.user.loginName
  ...
  methods: {
    godetails(e,item) {
      this.$router.push({name:'menjinAdd',query: {type:e,id: item.approvalId,}})
    },
    getnav(e) {
      if (this.active == e) {
        return
    ...
    getlist() {
      ...
      let parameter = {    //这里是发送给后台的数据
        ...
        visitStatuss: visitStatus,  //进入状态（1允许进入，2拒绝进入，3待审批）
        userId: this.userId,  //登陆用户手机号
      }
      postAction(approval, parameter).then((res) => {
